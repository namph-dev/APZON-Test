...
const BuyForm: React.FC = () => {
    const [lineItems, setLineItems] = useState<IBuyItem[]>([
        {
            id: uuidv4(),
            customerCode: "C001",
            productCode: "P001",
            documentDate: "2024-02-27",
    ...
    const saveOrder = () => {
        try {
            ...
            localStorage.setItem("buy", JSON.stringify([...buy, newOrder]));
            setLineItems([]);
        } catch (error) {
            ...
            render: (text: number, record: { id: string }) => (
                <InputNumber
                    ...
                    onChange={(value: number | null) =>
                        updateLineItem(record.id, "quantity", value ?? 0)
                    }
                />
            ),
        },
        {
            ...
            render: (text: number, record: { id: string }) => (
                <InputNumber
                    ...
                    parser={(value) =>
                        value ? parseFloat(value.replace(" VND", "")) : 0
                    }
                    value={text}
                    onChange={(value: number | null) =>
                        updateLineItem(record.id, "price", value ?? 0)
                    }
                />
            ),
        },
        ...
        },
        {
            ...
            render: (text: any, record: { id: string }) => (
                <Button
                    danger
                    onClick={() =>
                        setLineItems(lineItems.filter((item) => item.id !== record.id))
                    }
                >
                    Xóa
                </Button>
                ...
                <div className="head-order">
                    <FormItem label="Mã Nhà Cung Cấp">
                        <Input
                            ...
                            onChange={(e) =>
                                updateLineItem(firstLineItem.id, "customerCode", e.target.value)
                            }
                        />
                    </FormItem>
                    <FormItem label="Tên Nhà Cung Cấp">
                        <Input
                            ...
                            onChange={(e) =>
                                updateLineItem(firstLineItem.id, "productCode", e.target.value)
                            }
                        />
                    </FormItem>
                    <FormItem label="Ngày chứng từ">
                        <Input
                            ...
                            onChange={(e) =>
                                updateLineItem(firstLineItem.id, "documentDate", e.target.value)
                            }
                        />
                    </FormItem>
                </div>
